...
void LCD_Fill(u16 xsta, u16 ysta, u16 xend, u16 yend, u16 color) {
  u32 num = (xend - xsta) * (yend - ysta);
  LCD_Address_Set(xsta, ysta, xend - 1, yend - 1);  // 设置显示范围
  LCD_CS_Clr();
  
  // 创建颜色数据缓冲区
  static u16 color_buffer[1024];  // 使用静态缓冲区避免栈溢出
  u16 buffer_size = (num > 1024) ? 1024 : num;
  ...
  u32 remaining = num;
  while(remaining > 0) {
    u16 chunk_size = (remaining > buffer_size) ? buffer_size : remaining;
    
    // 使用DMA传输
    if(HAL_SPI_Transmit_DMA(&hspi2, (u8*)color_buffer, chunk_size * 2) == HAL_OK) {
      // 等待传输完成
...
void LCD_ShowPicture(u16 x, u16 y, u16 length, u16 width, const u8 pic[]) {
  u32 num = length * width * 2;
  LCD_Address_Set(x, y, x + length - 1, y + width - 1);
  LCD_CS_Clr();
  
  // 分块传输大图片
  const u8* pic_ptr = pic;
  u32 remaining = num;
  const u32 max_chunk = 8192;  // 最大传输块大小
  
  while (remaining > 0) {
    u32 chunk_size = (remaining > max_chunk) ? max_chunk : remaining;
    
    // 使用DMA传输
    if(HAL_SPI_Transmit_DMA(&hspi2, (u8*)pic_ptr, chunk_size) == HAL_OK) {
      // 等待传输完成
      while(HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY) {
        HAL_Delay(1);
      }
